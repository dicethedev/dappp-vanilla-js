...
async function getTokenDetails(id) {
  ...
  const token = await useContract(tokenList.tokens[id].address, abi);
  let userAddress = await signer.getAddress();
  ...
  try {
    ...
    let img = tokenList.tokens[id].logoURI;
...
async function InitData() {
  ...
  const template = "";
    tokenList.tokens.map( async (arrItem, index) => {
    const { name, symbol, img, totalSupply, userBalance } =  await getTokenDetails(index);
    
    template += tokenTemplateUpdate(name, symbol, img, totalSupply / 10 ** 18, 
    `${userBalance / 10 ** 18} ${symbol}`, arrItem.address);
